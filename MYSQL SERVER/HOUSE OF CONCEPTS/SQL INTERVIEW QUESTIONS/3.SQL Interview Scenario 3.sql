...
*/
--Scenario 3
----Write SQL query to fetch project-wise count of employees sorted by project’s count in descending order

-- Solution to retrieve count of employees for each project from the EmployeeSalary table
SELECT 
    Project, 
    COUNT(EmpId) AS NoOfEmployees
FROM 
    EmployeeSalary WITH (NOLOCK)
WHERE 
    Project IS NOT NULL
GROUP BY 
    Project
